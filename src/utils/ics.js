...
const PRODID = '-//mens-march-madness//EN'
...
function vevent(game, { now }) {
  ...
  const lines = [
    ...
    `UID:${game.id}@mens-march-madness`,
...
export function buildIcs(games, { name = "Men's March Madness", now = new Date().toISOString() } = {}) {
  const lines = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    `PRODID:${PRODID}`,
...
export function downloadIcs(games, { filename = 'mens-march-madness.ics', name } = {}) {
