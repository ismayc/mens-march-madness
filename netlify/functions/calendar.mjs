...
//
// Netlify Functions v2 (ESM default export). Query: ?teams=DUKE,ALA filters to those teams.
...
export default async (req) => {
  ...
  let name = `Men's March Madness ${SEASON}`
  if (teamsParam) {
    const want = new Set(
      teamsParam
        .split(',')
  ...
  return new Response(body, {
    ...
    headers: {
      ...
      'Content-Disposition': `inline; filename="mens-march-madness-${SEASON}.ics"`,
