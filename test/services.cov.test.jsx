...
// `paramount` is a real key in this repo's SERVICE_CATALOG; `bogus`/`gonesvc` are not.
beforeEach(() => {
  localStorage.clear()
})
...
function ServicesProbe() {
  ...
  return (
    <div>
      ...
      <span data-testid="has">{String(has('paramount'))}</span>
      <button onClick={() => toggle('paramount')}>paramount</button>
...
describe('services context — provider actions', () => {
  it('adds, ignores invalid keys, removes, and clears', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'paramount' })) // add
    expect(count()).toBe('1')
    expect(screen.getByTestId('has').textContent).toBe('true')
    expect(screen.getByTestId('list').textContent).toBe('paramount')

    await userEvent.click(screen.getByRole('button', { name: 'bogus' })) // not in catalog → no-op
    expect(count()).toBe('1')

    await userEvent.click(screen.getByRole('button', { name: 'paramount' })) // remove
    expect(count()).toBe('0')

    await userEvent.click(screen.getByRole('button', { name: 'paramount' })) // add again
...
describe('services context — fallback with no provider', () => {
  it('exposes inert has/toggle/clear that never throw or change state', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'paramount' }))
...
describe('services context — restoring from localStorage', () => {
  it('starts empty when the saved value is corrupt (lines 27-29)', () => {
    localStorage.setItem('mmm:services', 'not json')
    render(
      <ServicesProvider>
        <ServicesProbe />
      </ServicesProvider>
  ...
  it('starts empty when the saved value is valid JSON but not an array (line 26 : [])', () => {
    localStorage.setItem('mmm:services', JSON.stringify({ paramount: true }))
    render(
      <ServicesProvider>
        <ServicesProbe />
      </ServicesProvider>
  ...
  it('drops saved keys the catalog no longer defines', () => {
    localStorage.setItem('mmm:services', JSON.stringify(['paramount', 'gonesvc']))
    render(
      <ServicesProvider>
        <ServicesProbe />
      </ServicesProvider>
