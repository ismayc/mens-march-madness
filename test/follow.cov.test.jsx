...
describe('follow context — restoring from localStorage', () => {
  it('starts empty when the saved value is corrupt (mmm:followed)', () => {
    // The provider's real key is mmm:followed — set exactly that so JSON.parse throws
    // and the catch returns a fresh empty Set.
    localStorage.setItem('mmm:followed', 'not json')
    render(
      <FollowProvider>
        <FollowProbe />
      </FollowProvider>
