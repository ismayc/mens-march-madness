...
// A real played game from the committed bracket (UMBC @ HOW on truTV).
...
describe('GameDetail coverage', () => {
  ...
  it('renders watch chips for a game on the viewer’s services', () => {
    // truTV is a Turner network Max carries; select Max so the badge lights up.
    localStorage.setItem('mmm:services', JSON.stringify(['max']))
    render(
      <ServicesProvider>
    ...
    expect(watch).toHaveAccessibleName('Watch on Max')
  })
  ...
  it('labels multi-overtime periods (OT1/OT2) and suffixes the header /2OT', async () => {
    const twoOT = {
      ...
      score: [78, 75],
      ot: 2,
      line: { home: [30, 30, 10, 8], away: [28, 32, 10, 5] },
    }
    open(twoOT)
