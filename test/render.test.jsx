...
describe('GameCard', () => {
  ...
  it('labels games on the viewer’s chosen services and skips ones that are not', () => {
    // Viewer has YouTube TV and Paramount+.
    localStorage.setItem('mmm:services', JSON.stringify(['paramount', 'youtubetv']))
    ...
    // Paramount+ + CBS simulcast is watchable both ways — labels in catalog order.
    const { container, rerender } = render(
      withServices({ ...base, broadcast: ['Paramount+', 'CBS'] })
    )
    const watch = container.querySelector('.watch')
    expect(watch).toHaveAccessibleName('Watch on Paramount+, YouTube TV')
    expect([...watch.querySelectorAll('.watch-chip')].map((c) => c.textContent)).toEqual([
      'Paramount+',
      'YouTube TV',
    ])
    // "Paramount+" shows only as the badge, not repeated as a raw network; CBS (the
    // bundle's underlying network) still shows in the meta line.
    expect(container.querySelector('.game-meta').textContent).toContain('CBS')
    expect(screen.getAllByText('Paramount+')).toHaveLength(1)

    // A game only on a network the tournament doesn't use carries no badge.
    rerender(withServices({ ...base, broadcast: ['ESPN'] }))
  ...
  it('shows no service badge until the viewer picks services', () => {
    ...
    const { container } = render(<GameCard game={{ ...base, broadcast: ['CBS'] }} tz={TZ} />)
    expect(container.querySelector('.watch')).toBeNull()
  })
})
...
describe('ScheduleView', () => {
  ...
  describe('recent window and full season', () => {
    ...
    const games = [
      g('old', dOld, 'DUKE', 'BYU', [80, 70]),
      g('recent', dRecent, 'ALA', 'ARK', [88, 84]),
      g('today', today, 'ARIZ', 'AKR', [70, 66]),
      g('future', dFuture, 'CLEM', 'CBU'),
    ]
    const keysOf = (c) =>
    ...
    it('anchors on today when nothing is in the past', () => {
      const spy = Element.prototype.scrollIntoView
      render(
        <ScheduleView games={[g('today', today, 'ARIZ', 'AKR', [70, 66]), g('future', dFuture, 'CLEM', 'CBU')]} tz={TZ} />
      )
      expect(spy).toHaveBeenCalled()
    })
    ...
    it('does not scroll when no rendered day matches the anchor', () => {
      ...
      render(<ScheduleView games={[g('future', dFuture, 'CLEM', 'CBU')]} tz={TZ} />)
    ...
    it('falls back to the last week of games when the tournament is over', () => {
      ...
      const finished = [
        g('p1', shift(today, -40), 'DUKE', 'BYU', [80, 70]),
        g('p2', shift(today, -35), 'ALA', 'ARK', [88, 84]),
        g('p3', shift(today, -30), 'ARIZ', 'AKR', [70, 66]),
      ]
