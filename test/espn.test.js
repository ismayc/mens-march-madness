...
describe('fetchLive', () => {
  ...
  describe('normalizing status', () => {
    ...
    it('derives overtime periods past the second half', async () => {
      // College basketball is two halves, so regulation is period 2; anything beyond is OT.
      expect((await one({ period: 2 })).ot).toBeUndefined()
      expect((await one({ period: 3 })).ot).toBe(1)
      expect((await one({ period: 4 })).ot).toBe(2)
    })
