...
const draw = (games, followed = []) => {
  localStorage.setItem('mmm:followed', JSON.stringify(followed))
  return render(
    <FollowProvider>
...
describe('NextGame', () => {
  ...
  it('prefers a followed team’s next game over an earlier one', () => {
    draw(
      [
        game({ id: 'early', tip: '2026-03-19T17:00:00.000Z', home: 'ALA', away: 'ARK' }),
        game({ id: 'mine', tip: '2026-03-19T23:00:00.000Z', home: 'DUKE', away: 'BYU' }),
      ],
      ['DUKE']
    )
    expect(screen.getByText('⭐ Your next game')).toBeInTheDocument()
    expect(screen.getByText('Duke')).toBeInTheDocument()
    expect(screen.queryByText('Arkansas')).toBeNull()
  })

  it('stacks every game sharing the earliest tip when no team is followed', () => {
    draw([
      game({ id: 'a', home: 'DUKE', away: 'BYU' }),
      game({ id: 'b', home: 'ALA', away: 'ARK' }),
      game({ id: 'c', tip: '2026-03-19T23:00:00.000Z', home: 'ARIZ', away: 'AKR' }),
    ])
  ...
  it('a live game takes over from an upcoming one', () => {
    draw([
      game({ id: 'live', live: true, period: 2, statusLabel: '2nd 5:12' }),
      game({ id: 'later', tip: '2026-03-19T23:00:00.000Z', home: 'ALA', away: 'ARK' }),
    ])
  ...
  it('stacks several live games, with a period badge on each', () => {
    const { container } = draw([
      ...
      game({ id: 'l2', live: true, period: 2, statusLabel: '2nd 1:02', home: 'ALA', away: 'ARK' }),
    ])
  ...
  it('a followed team playing live wins outright over the other live games', () => {
    draw(
      [
        game({ id: 'l1', live: true, period: 1, home: 'ALA', away: 'ARK' }),
        game({ id: 'mine', live: true, period: 2, home: 'DUKE', away: 'BYU' }),
      ],
      ['BYU']
    )
    expect(screen.getByText('🔴 Live now')).toBeInTheDocument()
    expect(screen.getByText('BYU')).toBeInTheDocument()
    expect(screen.queryByText('Arkansas')).toBeNull()
  })
