...
// The committed 2026 bracket is a finished single-elimination tournament (Michigan
...
describe('buildBracket — the finished 2026 tournament', () => {
  it('is not projected and crowns Michigan the national champion', () => {
    expect(b.projected).toBe(false)
    expect(b.champion).toBe('MICH')
  })

  it('resolves all four region champions', () => {
    const champs = Object.fromEntries(b.regions.map((r) => [r.name, r.champion]))
    expect(champs).toEqual({ South: 'ILL', East: 'CONN', Midwest: 'MICH', West: 'ARIZ' })
  })

  it('names the four regions in the canonical order', () => {
    expect(b.regions.map((r) => r.name)).toEqual(REGIONS)
    expect(REGIONS).toEqual(['South', 'East', 'Midwest', 'West'])
  })

  it('pairs the Final Four semifinals (CONN v ILL) and (ARIZ v MICH)', () => {
    const pairs = b.finalFour.map((s) => s.teams.map((t) => t.abbr).sort())
    expect(pairs).toContainEqual(['CONN', 'ILL'])
    expect(pairs).toContainEqual(['ARIZ', 'MICH'])
    // Michigan and UConn win their semifinals to reach the final.
    const winners = b.finalFour.map((s) => s.winner).sort()
    expect(winners).toEqual(['CONN', 'MICH'])
  })

  it('runs the championship as MICH vs CONN, won by Michigan', () => {
    expect(b.championship.teams.map((t) => t.abbr).sort()).toEqual(['CONN', 'MICH'])
    expect(b.championship.winner).toBe('MICH')
  })
