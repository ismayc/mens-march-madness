...
describe('buildIcs', () => {
  ...
  it('gives each event a stable UID so re-import updates rather than duplicates', () => {
    ...
    expect(first).toContain(`UID:${played.id}@mens-march-madness`)
  })
...
describe('webcalUrl', () => {
  it('swaps https/http for the webcal scheme so a calendar app subscribes', () => {
    expect(webcalUrl('https://mens-march-madness.netlify.app/calendar.ics')).toBe(
      'webcal://mens-march-madness.netlify.app/calendar.ics'
    )
    expect(webcalUrl('http://x/y.ics')).toBe('webcal://x/y.ics')
  })

  it('leaves a query string (and its commas) intact', () => {
    expect(webcalUrl('https://host/calendar.ics?teams=DUKE,ALA')).toBe(
      'webcal://host/calendar.ics?teams=DUKE,ALA'
    )
  })
...
describe('googleCalendarUrl', () => {
  it('wraps a RAW (un-encoded) webcal URL in Google’s cid deep link', () => {
    expect(googleCalendarUrl('https://host/calendar.ics?teams=DUKE,ALA')).toBe(
      'https://www.google.com/calendar/render?cid=webcal://host/calendar.ics?teams=DUKE,ALA'
    )
  })
})
