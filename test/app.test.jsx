...
describe('App', () => {
  it('renders the shell and opens on the bracket', async () => {
    await mount()
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent(/Men's March Madness/)
  ...
  describe('my services', () => {
    it('opens the picker from the filter bar and remembers picks', async () => {
      ...
      await userEvent.click(within(dialog).getByLabelText(/Paramount/))
      expect(JSON.parse(localStorage.getItem('mmm:services'))).toContain('paramount')
    ...
    it('narrows the schedule to watchable games and remembers the choice', async () => {
      // Paramount+ carries only the CBS games; three of Michigan's six games are on CBS,
      // the rest on the Turner networks Paramount+ doesn't carry — so the toggle drops those.
      localStorage.setItem('mmm:services', JSON.stringify(['paramount']))
      scheduleAt('&team=MICH')
      await mount()
      ...
      expect(localStorage.getItem('mmm:watchOnly')).toBe('1')
      ...
      for (const card of document.querySelectorAll('.game')) {
        expect(within(card).getAllByText(/Paramount\+/).length).toBeGreaterThan(0)
      }
    })
  ...
  describe('past days', () => {
    ...
    it('remembers the choice per-device in localStorage', async () => {
      ...
      await waitFor(() => expect(localStorage.getItem('mmm:showPast')).toBe('0'))
    })

    it('restores a hidden preference from localStorage when the link says nothing', async () => {
      localStorage.setItem('mmm:showPast', '0')
      scheduleAt()
      await mount()
  ...
  describe('spoiler-free mode', () => {
    ...
    it('also remembers the choice per-device in localStorage', async () => {
      ...
      await waitFor(() => expect(localStorage.getItem('mmm:spoilerFree')).toBe('1'))
    })

    it('restores from localStorage when the link says nothing', async () => {
      localStorage.setItem('mmm:spoilerFree', '1')
      await mount()
      expect(screen.getByTitle('Spoiler-free mode')).toHaveAttribute('aria-pressed', 'true')
    })
  ...
  describe('theme', () => {
    it('flips the document attribute and persists it', async () => {
      ...
      expect(localStorage.getItem('mmm:theme')).toBe(after)
    })
  })
  ...
  describe('live alerts', () => {
    it('are off by default and persist when enabled', async () => {
      ...
      expect(btn).toHaveAttribute('aria-pressed', 'false')
      await userEvent.click(btn)
      expect(localStorage.getItem('mmm:alerts')).toBe('1')
    })
  })
  ...
  describe('the bracket', () => {
    it('crowns the national champion on the default view', async () => {
      ...
      expect(bar).toHaveTextContent(/Michigan Wolverines/)
    })
  })
