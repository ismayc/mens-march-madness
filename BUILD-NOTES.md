# the-mens-march-madness — build notes

An NCAA Division I Men's Basketball Tournament ("March Madness") bracket viewer, scaffolded
...
- **Two feed traps, both handled** (PLAYBOOK §2): the `seasontype=3` window also carries the
  **NIT** and the **College Basketball Crown** — filtered out by requiring the notes headline
  to start with `"NCAA Men's Basketball Championship"`; and the fetch **asserts the 67-game
...
- Each game carries `round` (FF4/R64/R32/S16/E8/FF/NC), `region` (South/East/Midwest/West, or
...
- **Repo scaffolded** from `the-nba-schedule`; identity fully substituted (NBA→MM: storage
  keys `mmm:*`, ESPN path `basketball/mens-college-basketball`, `.ics` domain, calendar host,
  canonical/OG URLs, footer, `package.json` name/scripts, theme accent to tournament crimson).
...
- **Bracket reconstruction** (`src/utils/bracket.js`, new): four regional sub-brackets
  ...
  the real 2026 data (champion MICH, 0 projected slots, every R64 seed pair sums to 17).
...
- **Icons + og-image**: custom orange hoop+net mark on flat crimson `#3a0d12` (per ICONS.md,
...
3. **Streaming-services filter** carries the NBA service catalog from the base; the games badge
   correctly against college networks (CBS/TBS/TNT/truTV) but the "my services" picker still
   lists NBA-flavored options. Low priority; adjust `utils/watch.js` if it matters.
