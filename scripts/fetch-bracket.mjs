...
// Men's Basketball Tournament ("March Madness") from ESPN's public scoreboard, and
...
const ESPN_PATH = 'basketball/mens-college-basketball'
const SITE = `https://site.api.espn.com/apis/site/v2/sports/${ESPN_PATH}`
// Only games whose notes headline starts with this belong to the bracket. The women's
// build swaps "Men's" → "Women's"; everything else (round names, structure) is identical.
const TOURNEY = /^NCAA Men's Basketball Championship/i
const GENDER_LABEL = "Men's"
...
const REGION_RE = /\b(West|East|South|Midwest)\b\s*Region/i

function parseBracket(headline) {
  const round = ROUND_PATTERNS.find(([re]) => re.test(headline))?.[1]
  const region = headline.match(REGION_RE)?.[1] || null // null for Final Four + Championship
  return { round, region }
}
...
function normalizeEvent(ev) {
  ...
  const score = completed
    ...
    : undefined
  // College regulation is two halves (period 2); anything beyond is overtime.
  const otPeriods = c.status?.period > 2 ? c.status.period - 2 : undefined
